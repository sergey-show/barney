...
Инструменты — протокол ядра. Модель их не выдумывает: код даёт фиксированный набор, выполняет вызов и **сверяет** результат до следующего шага.
...
| Семейство | Инструменты | Для чего |
|---|---|---|
| Файлы | `fs_list`, `fs_stat`, `fs_read`, `fs_write`, `fs_edit`, `fs_append`, `fs_mkdir`, `fs_search`, `fs_remove`, `fs_restore` | Файлы в worktree. Для правок лучше это, а не shell. |
...
| Пробуждение | `wake_when_process`, `wake_at`, `wake_list`, `wake_cancel` | Отложить ход: выход/лог процесса или время. См. [Wake](wake.ru.md). |
| Поиск | `web_search` | Публичный веб. Потом открыть источники в браузере. |
...
| Самость | `self_status`, `self_log`, `self_commit`, `self_rollback` | Git в `~/.barney`, не ядро. |

Старые алиасы `skill_*` для plugin-инструментов ещё работают.
...
Секреты в выводе инструментов становятся `DETECTED_SECRET_<KIND>_<HASH>`. В `fs_edit` / `sed` нужен **полный токен**. Поиск по буквам `DETECTED_SECRET` живые секреты на диске не найдёт.

Тот же провалившийся вызов блокируется. Семейство, которое упало дважды, насыщается: надо сменить семейство, а не долбить тот же инструмент.

`fs_restore` возвращает последнюю удачную версию файла в этой сессии после плохой записи.

## Короткие ходы

Приветствие и «кто ты» могут обойтись без инструментов. Всё, где нужен файл, shell, браузер, плагин или факт о мире, идёт полным циклом: план → инструмент → сверка → ревью.
